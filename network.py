...
class ResidualGroup(nn.Module):
    def __init__(self, conv, n_feat, kernel_size=3, reduction=16, act=nn.ReLU(True), res_scale=1, n_resblocks=8):
    ...
    def __init__(self, conv, scale, n_feat, bn=False, act=False, bias=True, n_resblocks=4):
        ...
        if (scale & (scale - 1)) == 0:    # Is scale = 2^n?
            for _ in range(int(math.log(scale, 2))):
                ...
                m.append(ResidualGroup(conv, n_feat, n_resblocks=n_resblocks))
                if bn: m.append(nn.BatchNorm2d(n_feat))
                if act: m.append(act())
        elif scale == 3:
...
class FCN_Classifier(nn.Module):
    ...
    def forward(self, fea):
        fea = self.down_conv_1(self.relu(self.block_1(fea)))
        fea = self.down_conv_2(self.relu(self.block_2(fea)))
        out = self.linears(fea)
...
class RCAN(nn.Module):
    def __init__(self, opt_net, conv=default_conv):
        ...
        # define tail module for F-G Classification branch
        
        # define tail module for Super Resolution branch
        modules_tail_SR = [
            Upsampler(conv, scale, n_feats, act=False, n_resblocks=n_resblocks),
